...
var PreparedTemplateBuilder = require('../lib/PreparedTemplateBuilder');
...
describe("Prepared Javascript", function () {
  function buildTestData() {
    ...
    var output = function (text, originIndex) {
      return util.format(_output, text, originIndex);
    };
    var indentPart = function (text, originIndex) {
      return util.format(_indentPart, text, originIndex);
    };
    var expression = function (text, originIndex) {
      return util.format(_expression, originIndex, text, originIndex);
    };
    ...
    return [
      ...
      {name: "no expr", template: "|#(test", js: indentPart("", 0) + output("#(test", 1) + newLine},
      {
        name: "escaped expr",
        template: "|##{test",
        js: indentPart("", 0) + output("#", 1) + output("{test", 2) + newLine
      },
      {name: "single expr 1", template: "|#{test}", js: indentPart("", 0) + expression("test", 1) + newLine},
      {
        name: "single expr 2",
        template: "|#{test}}",
        js: indentPart("", 0) + expression("test", 1) + output("}", 2) + newLine
      },
      {
        name: "inline expr 1",
        template: "|simple #{test.prop} expr",
        js: indentPart("", 0) + output("simple ", 1) + expression("test.prop", 2) + output(" expr", 3) + newLine
      },
      {
        name: "logic expr",
        template: "|#{ (i==0 ? ':' : \",\") }",
        js: indentPart("", 0) + expression(" (i==0 ? ':' : \",\") ", 1) + newLine
      },
      {
        name: "escaped expr 1",
        template: "|escape ##{test} expr",
        js: indentPart("", 0) + output("escape #", 1) + output("{test} expr", 2) + newLine
      },
      {
        name: "escaped expr 2",
        template: "|###{test}",
        js: indentPart("", 0) + output("#", 1) + expression("test", 2) + newLine
      },
      {name: "string expr 1", template: "|#{\"}\"}", js: indentPart("", 0) + expression("\"}\"", 1) + newLine},
      {
        name: "string expr 2",
        template: "|#{'\\'\\\"}'}",
        js: indentPart("", 0) + expression("'\\'\\\"}'", 1) + newLine
      },
...
describe("Prepared SourceMap", function () {
  function buildTestData() {
    return [
      //                                                1         2
